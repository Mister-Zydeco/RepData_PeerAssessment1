...
```
The time series of daily totals has a mean of `r sprintf('%.2f', meanSteps)` and a median of `r medianSteps`.
...
Averaged over all days in the two-month sample period, the subject took `r maxAverageSteps` steps  between `r tod0Str` and `r tod1Str`. This was the most active of all five-minute time-of-day intervals and is depicted in red on the plot above.

## Imputing missing values
There are `r sum(is.na(stepData$steps))` five-minute intervals in the two months of data for which the step count is missing. We turn to the question of imputing values where they are missing: we take the simple-minded approach that for any record with a missing step count, we will impute the mean step count for that record's interval. 
...
Imputing values to the intervals where step counts were missing clearly will increase the daily step totals. After we impute missing data, the time series of daily total steps has a mean of `r sprintf('%.2f', meanAugSteps)` and a median of `r sprintf('%.2f', medianAugSteps)`.
